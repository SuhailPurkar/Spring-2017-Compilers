...
public class Parser {
    ...
    private JAST typeDeclaration() {
        ...
        while (have(SEMI));
        if(see(INTERFACE)){
        	return interfaceDeclaration(mods,true);
        }
        else {
        	return classDeclaration(mods, true);
        }
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods, boolean flag) {
        int line = scanner.token().line();
        mustBe(CLASS);
        mustBe(IDENTIFIER);
        ...
        return new JClassDeclaration(line, mods, name, 
        		superClass, interfaces , classBody());
    ...
    //hw3 added interface declaration
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods, boolean flag) 
    {
    	int line = scanner.token().line();
    	mustBe(INTERFACE);
    	mustBe(IDENTIFIER);
  ...
  //hw3 added interface body
    ...
    //hw3 interface member declaration.
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        ...
        if (seeIdentLParen()) {
        	...
        	interfaceMemberDecl = classDeclaration(mods, false);
        }
        else if (see(INTERFACE)){ //inner class
        	interfaceMemberDecl = interfaceDeclaration(mods, false);
        }
        else {
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        //hw3 added
        	...
        	memberDecl = classDeclaration(mods, false);
        }
        else if (see(INTERFACE)){ //inner class
        	memberDecl = interfaceDeclaration(mods, false);
        }
        else {
...
//        } 
    ...
    private JExpression conditionalExpression() {
        ...
        while (more) {
            if (have(QUESTION)) {
                JExpression correct = assignmentExpression();
                mustBe(COLON);
                condition = new JQuestionOp(line, condition, correct, conditionalExpression());
            } else {
                more = false;
            }
        }
