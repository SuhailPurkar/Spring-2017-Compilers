    ...
    public TokenInfo getNextToken() {
        ...
        while (moreWhiteSpace) {
            ...
            if (ch == '/') {
                ...
                //HW2 Added Multi-line	
                	while (ch != EOFCH){
                		nextCh();
                			while (ch == '*'){ 
                				// * needs / to seal comment
                				nextCh();
                				if (ch == '/'){ 
                					//seals multiline comment
                					break;					
                				}	
                		} 
                			nextCh();
        ...
        case '>':
            ...
            if (ch == '=') {
				...
				if (ch == '>') {						
            		...
            		if (ch == '=') {					
            			nextCh();
            			return new TokenInfo(ZRS_ASSIGN, line);
            		}
            		else {
            		return new TokenInfo(ZRS, line);
            		}
            	}
            	else if  (ch == '=') {					
            		nextCh();
...
//          return new TokenInfo(INT_LITERAL, buffer.toString(), line);
        	...
        	boolean seeHex = false;      	
        	boolean seeExp = false;
        	boolean seePow = false;
        	boolean expSign = false;
        	boolean seeInt = true;
        	
        	//classify number into type
        	if (seeInt){
        		return new TokenInfo(INT_LITERAL, buffer.toString(), line);
        	}
        	else {
        		if (priorChar(buffer, 'l') || priorChar(buffer, 'L')) {
        		if (seeDot || seeExp || seePow || expSign){
        			reportScannerError(buffer.toString());
        		} else {
        		return new TokenInfo(LONG_LITERAL, buffer.toString(), line);
        		}
        		}	
        	}
        	if (priorChar(buffer, '-') || priorChar(buffer, '+')) {
        			reportScannerError(buffer.toString());
        		
        	}
        	else{ 
        			if (priorChar(buffer, 'f') || priorChar(buffer, 'F')) {
	        		if (!seePow && seeHex && seeDot){
	        			reportScannerError(buffer.toString());
	        		} else {
	        	...
	        	if (priorChar(buffer, 'e') || priorChar(buffer, 'E') ||
	        				priorChar(buffer, 'p') || priorChar(buffer, 'P')){
	        			reportScannerError(buffer.toString());
	        		} 
	        	else {
    ...
    private boolean priorChar(StringBuffer buffer, char ch) {
    	return (buffer.charAt(buffer.length() -  1) == ch);
    }
