...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////	
//HW3 interface implementation
	//Interace implementing
	private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods){

        int line = scanner.token().line();
        ArrayList<TypeName> extended = new ArrayList<TypeName>();
     	
		//Report an error if there is a static or private modifier becaues
		//it can only be public
        if((mods.contains("static") || mods.contains("private") 
					                             || mods.contains("final"))){
													 
     reportParserError("Interface can't have static private or final\n");
		...
		//Ignore the commas and add all of the classes it extends
        if (have(EXTENDS)){
			extended.add(qualifiedIdentifier());
			while(have(COMMA)){
  ...
  return new JInterfaceDeclaration(line, mods, name, extended, interfaceBody());
	...
	private ArrayList<JMember> interfaceBody() {
        ...
        while (!see(RCURLY) && !see(EOF)) {
            ...
            else members.add(interfaceMemberDecl(modifiers()));
        }
        mustBe(RCURLY);
        return members;
    }
	...
	//the interface member declerator
	private JMember interfaceMemberDecl(ArrayList<String> mods) {
        int line = scanner.token().line();
        JMember interfmemberDecl = null;
        if (see(CLASS)) {
            // A constructor
			//interfmemberDecl = classDeclaration(mods);
        } 
			
			if(see(INTERFACE)){
				interfmemberDecl = interfaceDeclaration(mods);
			}
            ...
            JBlock body = block();
            interfmemberDecl = new JConstructorDeclaration(line, mods, name, params,
                    body);
			}
			else {
            Type type = null;
             if (have(VOID)) {
                ...
                interfmemberDecl = new JMethodDeclaration(line, mods, name, type,
                        params, body);
            } else {
                type = type();
                if (seeIdentLParen()) {
                    ...
                    // Field
                    interfmemberDecl = new JFieldDeclaration(line, mods,
                            variableDeclarators(type));
                    mustBe(SEMI);
                }
            }
        }
        return interfmemberDecl;
    }
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
		...
		ArrayList<TypeName> implements_list = new ArrayList<TypeName>();
		//Ignore the commas and add all of the interfaces it implements
        if (have(IMPLEMENTS)){
            do { implements_list.add(qualifiedIdentifier());
            } while (have(COMMA)); 
        }
	
        return new JClassDeclaration(line, mods, name, superClass, implements_list, classBody());
    }
	...
	//IF A CRULY brace is found, it could be a instance or a static block
	else if (have(LCURLY)) { 
			//get the body of the static block
			JBlock body;
			//if there is an semi colon then its empty
			if(have(SEMI)){
				body = null;
			}
			//else get the block
			...
			//if there is a static modifier then its a static block
			if(mods.contains(STATIC.image())){
				//return the new static block
				memberDecl = new JStaticBlock(line, mods, body);
			}
			//else its an instance block
			else{
				//return an instance block
				memberDecl = new JInstanceBlock(line, mods, body);
			}
    ...
    private JStatement statement() {
		...
		/*else if(see(COLON)){
			...
			return new JLabelStatement(line, body);
		}*/
		else if(have(BREAK)){
			//JStatement label = statement();
			mustBe(SEMI);
		...
		//	JStatement label = statement();
			...
			return new JContinueStatement(line);
				...
				JStatement body = statement();
				//syntax after must be a while
				mustBe(WHILE);
				//get the condition for the loop
				JExpression condition = parExpression();
				//return the do while node
				return new JDoWhileStatement(line, condition, body);
				
				
		}
			else if(have(FOR)){
				//Check for beginning of parantheses
				mustBe(LPAREN);
				
				//Variable could be initialized inside or outside of for loop
				//So check if it is or not
				
				
				//Get the initialize expression
		 JVariableDeclaration initialize = localVariableDeclarationStatement();
		 //Check if there is a colon after the initialized, then it is a 
		 //enhanced for loop
		 if(have(COLON)){
			 	//There might be a expression so check if not null
				JExpression expression = expression();
				
				//must be a closing parentheses
				mustBe(RPAREN);
		  ...
		  return new JEnhancedForStatement(line, initialize, expression, body);
		 }
		 //Else it is a regular for loop
		 else{
				//There might be a expression so check if not null
				JExpression expression = expression();
				...
				//return the new for loop node
		  return new JForStatement(line, initialize, expression, update, body);
		 }
			}
    ...
    private JExpression conditionalAndExpression() {
        ...
        while (more) {
            if (have(LAND)) {
				...
				JExpression mhs = conditionalAndExpression();
				//Must be a colon the divides the two operands else error
				mustBe(COLON);
				//Could be another conditional expression
				JExpression rhs = conditionalAndExpression();
                lhs = new JTernaryOp(line, lhs, mhs, rhs);
            }
    ...
    private JExpression conditionalOrExpression() {
	    ...
	    private JExpression conditionalTernaryExpression() {
        int line = scanner.token().line();
        boolean more = true;
...
private JExpression multiplicativeExpression() {
    ...
    private JExpression unaryExpression() {
        ...
        }  else if (have(PLUS)) {	//Adding in case if a unary + expression is found, added in for HW1
    ...
    private JExpression literal() {
		...
		//New Double literal added for hw3
